...
// Checks banned words, invite links, mass mentions and spam in that order
// (first hit wins) — deletes the message and issues an automatic warning,
// which feeds straight into the same escalation ladder as a manual /mod
// warn (see actions.js's checkEscalation).
async function handleAutomod(message, cfg) {
  let violation = null;

  const bannedWord = findBannedWord(message.content, cfg.automod_words);
