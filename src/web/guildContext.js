...
// working), and gates every module route behind server-admin status except
// a short allowlist that must keep working regardless (identity and the
// guild picker).

const guards = require('../core/guards');

const UNGATED_PREFIXES = ['/me', '/guilds'];
...
// before the admin gate below (an anonymous request should get a 401, never
// a 400/403 that reveals anything about guild/admin state).
...
module.exports = { requireAuth, resolveGuildId, requireGuildAdmin };
